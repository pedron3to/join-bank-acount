...
describe("BankAccount", function () {
  ...
  async function deployBankAccountWithAccounts(
    ...
    if (owners == 2) {
      ...
      address = [addr1.address, addr2.address];
    } else if (owners == 4) {
      address = [addr1.address, addr2.address, addr3.address];
    }

    await bankAccount.connect(addr0).createAccount(address);
    ...
    for (const withdrawlAmount of withdrawslAmounts) {
  ...
  describe("Creating an account", () => {
    ...
    it("should allow creating a quad user account", async () => {
      const { bankAccount, addr0, addr1, addr2, addr3 } =
        await loadFixture(deployBankAccount);
      await bankAccount
        .connect(addr0)
  ...
  describe("Withdraw", () => {
    describe("Request a withdraw", () => {
      ...
      it("non-account owner can NOT request withdraw", async () => {
        ...
        await expect(bankAccount.connect(addr1).requestWithdrawl(0, 190)).to.be
          .reverted;
      });
